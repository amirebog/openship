...
export function Navbar() {
  ...
  return (
    <header className="fixed top-0 z-[60] w-full">
      <div className="relative mx-auto flex max-w-7xl items-center justify-between px-6 pt-5">
        {/* ── Logo ────────────────────────────────────────────── */}
        ...
        <div className="relative z-50 flex shrink-0 items-center gap-3">
          ...
          <button
            ...
            className="relative z-20 flex h-10 w-10 items-center justify-center rounded-full md:hidden"
            style={{
      ...
      {mobileOpen && (
        <div className="fixed inset-0 z-40 md:hidden" aria-modal="true">
          ...
          {/* Panel */}
          <nav
            className="absolute inset-y-0 end-0 flex h-full w-[280px] flex-col gap-1 bg-[var(--th-bg-page)] px-6 pt-24 shadow-2xl"
          >
            {NAV_ITEMS.map((item) => (
              <Link
                key={item.label}
